...
def main() -> None:
    ...
    # Inicializar agente con integración Ollama
    ollama = OllamaClient(model="gemma3:4b")
    agent = DungeonLifeAgent(language_model=ollama)
    print("Willow listo con integracion Ollama. Escribe 'salir' para terminar.")
    while True:
        try:
        ...
        if lowered.startswith("memoria registrar"):
            ...
            if len(fields) < 4:
                print("Formato esperado: memoria registrar canal;autor;resumen;contenido[;tags][;decisiones]")
                print()
                continue
        ...
        if lowered.startswith("memoria buscar"):
            ...
            if not results:
                ...
                for record in results:
                    ...
                    print(f"[{record.timestamp}] {record.channel} · {record.author} → {record.summary} ({tags})")
            print()
            continue
        ...
        if lowered.startswith("productividad"):
            ...
            try:
                ...
                agent.register_productivity(role=role, tasks_completed=tasks, session_minutes=minutes)
